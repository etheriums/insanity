...
async def on_message(message):
    for word in editWords:
        if word in message.content.lower() and filterWords:
            time.sleep(3.5)
            try:
                return await message.edit(content="[>] i follow tos [<]", delete_after=3)
            except: pass
...
[ Made by: sai#8000 ]```""".format(snipe_message_author[ctx.channel.id], f"#{ctx.channel.name}", snipe_message_content[ctx.channel.id]), delete_after=5)
    except KeyError:
...
[ Made by: sai#8000 ]```""".format(message), delete_after=5)
...
async def purge(ctx, amount: int):
    if amount > 200:
        return await ctx.reply("dumbass, dont purge more than 200.", delete_after=3)
...
[ Made by: sai#8000 ]```""".format(deleted, slept, ratelimited), delete_after=5)
